...
export function ReadinessEvidenceLifecyclePanel({
  ...
  return (
    <section className="grid gap-3 lg:grid-cols-3" aria-label="Evidence lifecycle status">
      <div className="rounded-lg bg-hush-bg-light/55 p-4">
        <h2 className="text-lg font-semibold">Evidence Lifecycle</h2>
        <div className="mt-4 grid grid-cols-2 gap-2">
          {lifecycleOrder.map((status) => (
            <div key={status} className="rounded-md bg-hush-bg-dark/50 px-3 py-2">
      ...
      <div className="rounded-lg bg-hush-bg-light/55 p-4">
        ...
        {dashboard.staleEvidence.length === 0 ? (
          <p className="mt-4 rounded-md bg-hush-bg-dark/50 px-3 py-3 text-sm text-hush-text-accent">
            No stale or superseded evidence in the promoted register.
          </p>
        ) : (
          <div className="mt-4 grid gap-2">
            {dashboard.staleEvidence.map((evidence) => (
              <div key={evidence.evidenceId} className="rounded-md bg-amber-300/14 px-3 py-3 text-sm text-amber-100">
        ...
        {dashboard.exceptions.length === 0 ? (
          ...
          <div className="mt-4 grid gap-2">
            {dashboard.exceptions.map((exception) => (
              <div key={exception.exceptionId} className="rounded-md bg-amber-300/14 px-3 py-3 text-sm text-amber-100">
