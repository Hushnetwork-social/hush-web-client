...
function buildRequest(
  ...
  if (typeof limit === "number" && Number.isFinite(limit) && limit > 0) {
    bytes.push(...encodeVarintField(2, Math.trunc(limit)));
  }
  if (beforeEntryId) {
    bytes.push(...encodeString(3, beforeEntryId));
  }
  if (requesterPublicAddress) {
    bytes.push(...encodeString(4, requesterPublicAddress));
  }
  bytes.push(...encodeVarintField(5, isAuthenticated ? 1 : 0));
...
function parseResponse(messageBytes: Uint8Array): SocialCommentsPageDto {
  const result: SocialCommentsPageDto = {
    success: false,
    message: "",
    comments: [],
  ...
  while (offset < messageBytes.length) {
    ...
    if (wireType === 2) {
      ...
      if (fieldNumber === 2) {
        ...
        result.comments.push(parseThreadEntry(fieldBytes));
      } else if (fieldNumber === 4) {
        let nestedOffset = 0;
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!postId) {
      return NextResponse.json({ success: false, message: "Missing postId", comments: [], paging: { initialPageSize: 10, loadMorePageSize: 10 }, hasMore: false }, { status: 400 });
    }
    ...
    const requestBytes = buildRequest(postId, limit, beforeEntryId, requesterPublicAddress, isAuthenticated);
    const responseBytes = await grpcCall("rpcHush.HushFeed", "GetSocialCommentsPage", requestBytes);
    const messageBytes = parseGrpcResponse(responseBytes);
    if (!messageBytes) {
      return NextResponse.json({ success: false, message: "Invalid gRPC response", comments: [], paging: { initialPageSize: 10, loadMorePageSize: 10 }, hasMore: false }, { status: 502 });
    }

    return NextResponse.json(parseResponse(messageBytes));
  } catch (error) {
    console.error("[API] GetSocialCommentsPage failed:", error);
    return NextResponse.json({ success: false, message: "Failed to fetch social comments", comments: [], paging: { initialPageSize: 10, loadMorePageSize: 10 }, hasMore: false }, { status: 502 });
  }
}
